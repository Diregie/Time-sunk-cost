...
def show_graph(flag=0):
    ...
    nx.draw_networkx_edges(G, pos, width=0.8, alpha=0.2)
            ...
            if v_neighbors:
                ...
                if abs(G.node[v]['value']) <= 0.3 and abs(G.node[w]['value']) <= 0.3:
                    if abs(G.node[v]['value']) > abs(G.node[w]['value']):
                        ...
                        G.node[v]['value'] = sunk_parameter_v * (1 + rho) * (G.node[w]['value'] + G.node[v]['value']) / 2
                        ...
                        G.node[w]['value'] = sunk_parameter_w * (1 + rho) * (G.node[w]['value'] + G.node[v]['value']) / 2
                        sunk_round(pre_opinion_v, G.node[v]['value'], v)
                        sunk_round(pre_opinion_w, G.node[w]['value'], w)
                        color_map[int(v - 1)] = G.node[v]['value']
...
def plot_comparison_test(op1, op_eps):
    ...
    # N = op1.shape[1]
    for i in range(N):
        plt.plot(op1[:, i], c1, alpha=0.3, linewidth=1)
    # plt.vlines(special_rounds, 0.0, 1.0, linewidth=5)
    p1 = patches.Patch(color=c1, label='HK model')
    legend = plt.legend(shadow=True, frameon=True, loc='upper right')
...
def statistics_graph():
    ...
    for i in range(8):
        plt.plot(staData[:, 0], "green", alpha=0.2, linewidth=0.5)
        plt.plot(staData[:, 1], "yellow", alpha=0.3, linewidth=0.5)
        plt.plot(staData[:, 2], "red", alpha=0.2, linewidth=0.5)
        plt.plot(staData[:, 3], "blue", alpha=0.2, linewidth=0.5)
        plt.plot(staData[:, 4], "green", alpha=0.6, linewidth=0.5)
        plt.plot(staData[:, 5], "orange", alpha=0.6, linewidth=0.5)
        plt.plot(staData[:, 6], "red", alpha=0.6, linewidth=0.5)
        plt.plot(staData[:, 7], "purple", alpha=0.6, linewidth=0.5)
    p1 = patches.Patch(color="green", label='wait_and_see state')
    p2 = patches.Patch(color="yellow", label='subjective state')
    p3 = patches.Patch(color="red", label='firm_state')
    p4 = patches.Patch(color="blue", label='extreme_state')
    legend = plt.legend(handles=[p1, p2, p3, p4], shadow=True, frameon=True, loc='upper left')  # 标注
    legend.get_frame().set_facecolor('#FFFFFF')
...
# G = nx.barabasi_albert_graph(N, 225)    # Barabási–Albert无标度图
all_degrees()
color_map = []  # 意见表
